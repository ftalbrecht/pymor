...
def pod_block_basis_extension(basis, U, count=1, copy_basis=True, product=None, orthonormalize=True, require_all=False):
    """Block variant of |pod_basis_extension|.
    """

    logger = getLogger('pymor.algorithms.blockbasisextension.pod_block_basis_extension')
    ...
    for ii in np.arange(num_blocks):
        try:
            nb, ed = pod_basis_extension(basis[ii],
                                         blocks[ii],
                                         count=count,
                                         copy_basis=copy_basis,
                                         product=product[ii],
